...
async function start() {
    ...
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task_ndadcx('开始查询'));
        await wait(2);
    }
    await Promise.all(taskall);
    console.log('\n================== 查询扭蛋抽奖次数 ==================\n');
    ...
    taskall = [];
    for (let user of userList) {
        taskall.push(await user.task_zx('开始资讯任务'));
        await wait(2);
    }
    await Promise.all(taskall);
    console.log('\n================== 运行后金币查询 ==================\n');
...
class UserInfo {
    constructor(str) {
        ...
        this.ck2 = this.cktest2;
            ...
            this.headers = {
                ...
                'User-Agent': 'Mozilla/5.0 (Linux; Android 7.1.2; X50 Build/N6F26Q; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.117 Mobile Safari/537.36;_android{"version":6920,"versionName":"6.9.20","userType":"1","sdkVersion":"25","statusBarHeight":23,"toolBarHeight":67,"oaid":"","channel":"vivo","uid":"5HYTM3"}_android',
    ...
    async task_ndcx(name) { // 扭蛋次数查询
        try {
            let options = {
                method: "get",
                url: `https://market-gateway.julanling.com/market-center/api2/gacha/index?deviceToken=4e978751b3e56331d11bf8f6a0ff95fd&version=6.9.20&os=ANDROID&appVersion=6.9.20&appChannel=unknow`,
                headers: {
                    Authorization: this.ck,
            ...
            //console.log(result);
            if (result.errorCode == 0) {
                DoubleLog(`账号[${this.index}]  查询: ${result.data.remainVideoTimes}`);
            } else {
    ...
    async task_ndadcx(name) { // 扭蛋次数查询
        try {
            ...
            let result = await httpRequest(options, name);
            if (result.errorCode == 0) {
                DoubleLog(`账号[${this.index}]  开始查询: ${result.results.remainVideoTimes}`);
                let remainVideoTimes = parseInt(result.results.remainVideoTimes)
                if (remainVideoTimes > 0) {
                    console.log("判断当前可观看广告" + remainVideoTimes + "次,开始为你看广告增加次数");
                    for (let i = 0; i < remainVideoTimes; i++) {
                        await this.task_ndgg("看广告");
                        await wait(6);
                    }
                }
            }
    ...
    async task_ndgg(name) { // 扭蛋广告
        try {
            let options = {
                method: 'POST',
                url: 'https://market-gateway.julanling.com/market-center/api2/gacha/finishGachaTask',
                headers: this.headers,
                body: `{"businessType":"JJB_DAILY_GACHA_INC_VIDEOS","deviceToken":"4e978751b3e56331d11bf8f6a0ff95fd","version":"6.9.20","os":"ANDROID","appVersion":"6.9.20","appChannel":"unknow"}`,
            };
            //console.log(options);
            let result = await httpRequest(options, name);
            //console.log(result);
            if (result.errorCode == 0) {
                DoubleLog(`账号[${this.index}]  增加: ${result.results.amount}次抽奖`);
            } else {
                DoubleLog(`账号[${this.index}]  失败 ❌ 了呢,原因` + result.errorStr);
                //console.log(result);
            }
        } catch (error) {
            console.log(error);
