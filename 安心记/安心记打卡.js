...
const $ = new Env("安心记打卡投入");
const ckName = "axj";
let show = "投入200金币 投稿群--908013361"
//-------------------- 一般不动变量区域 -------------------------------------
...
async function start() {
    ...
    for (let user of userList) {
        taskall.push(await user.info_point('开始投入金币'));
        await wait(2);
    }
...
class UserInfo {
    constructor(str) {
        ...
        this.body = `{"period":"${nowDate}","os":"ANDROID","appVersion":"6.9.20","appChannel":"vivo","deviceToken":"4e978751b3e56331d11bf8f6a0ff95fd"}`

    }

    async info_point(name) { // 投200金币
        try {               //这是点击去打卡不然无法领奖
            let options = {
                method: "post",
                url: `https://market-gateway.julanling.com/market-center/api2/assignment/finishAssignment`,
                headers: this.headers,
                body: `{"businessType":"JJB_CLOCK_OUT_DAY_TASK_CLICK","os":"ANDROID","appVersion":"6.9.20","appChannel":"vivo","deviceToken":"fcb7f493b48007c82dc398031ba3f276"}`,
        ...
        try {
            ...
            if (result.errorCode == 0) {
                //let amount = result.results.openBoxAwards[0].amount
                //console.log(amount)
                DoubleLog(`账号[${this.index}]  投入200金币: ${result.results.resultType}`);
            } else {
                DoubleLog(`账号[${this.index}]  投入失败 ,原因` + result.errorStr);
                console.log(result);
            }
        } catch (error) {
            console.log(error);
