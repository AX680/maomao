...
async function start() {
    ...
    for (let user of userList) {
        taskall.push(await user.user_info('打卡'));
        //await wait(1); //延迟
    }
...
class UserInfo {
    constructor(str) {
        ...
        this.body = `{"period":"${nowDate}","os":"ANDROID","appVersion":"6.9.20","appChannel":"vivo","deviceToken":"fca221837b43677f75f2427076822808"}`
        //let ck = str.split('&')
        //this.data1 = ck[0]
    ...
    async user_info(name) { // 打卡
        try {
            ...
            if (result.errorCode == 0) {
                DoubleLog(`账号[${this.index}]  打卡时间: ${result.results.clockTime} ${result.results.chickenSoup}`);
            } else {
                DoubleLog(`账号[${this.index}]  打卡失败 ❌ 了呢,原因${result.errorStr}`);
                console.log(result);
            }
        } catch (error) {
            console.log(error);
